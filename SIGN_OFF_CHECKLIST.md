...
| # | Assumption | Config key / Code location | Status |
|---|---|---|---|
| A7 | The structural fair-value (P\*) equation uses three regressors: log real income, log rent, and the current mortgage rate. These were chosen on theoretical grounds. No formal variable selection procedure was run. | `config/parameters.json` → `fair_value.regressors` | Expert-prior — economically defensible; not formally tested against alternatives |
...
| A28 | The simulation baseline conservatism noted in validation (simulated median 5yr return 3.07% vs historical mean 18.05%) reflects the deliberate conservative calibration and the 2022-2025 rate-shock conditioning, not a model error. This assumption has not been formally tested. | `SIMULATION_RECALIBRATION_NOTE.md`; `region_plausibility_diagnostics.csv` | Expert-prior — levels are not validated; scenario ordering is validated |
...
| `sigma_floor_overrides` | Not implemented | Per-region sigma floor override (analogous to sigma_cap_overrides). | Required to address IQR compression in Wales and Yorkshire. | Investigation 2026-03-22: required floors for Wales (0.115) and Yorkshire (0.115) both exceed the 0.035 governance threshold. Override infrastructure NOT implemented — required values would approach the global cap (0.12) and would not represent a true floor. See R020 and R021. |
...
**L17 — The simulation baseline median substantially underestimates the historical 5-year return distribution.**
Independent validation shows the simulated baseline median 5-year return is 3.07% against a historical mean of 18.05% across UK regions. The simulation is useful for scenario ordering (which scenarios produce better/worse outcomes relative to each other) and for tail-risk assessment, but the absolute level of the simulated paths should not be read as a calibrated price forecast.
...
| 3 | Statistical Estimation (OLS / Fair Value / P\*) | Luqman — Student | 2026-03-31 | APPROVED | OLS estimation with HAC maxlags=24. Guardrail overrides <40%. Rate channel documented via stability diagnostics. P* CI added. |
| 4 | Simulation Calibration (SDE / Stage 5) | Luqman — Student | 2026-03-31 | APPROVED | SDE calibrated. IQR compression in Wales/Yorkshire formally accepted per R020/R021. Sigma cap at 0.120 documented. |
